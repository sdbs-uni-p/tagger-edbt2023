    ...
    for entry in candidate_list:
        ...
        for v in versions:
            if len(v) > 0 and cand_then.startswith(v):
                ...
                break
        loc_vers = list(set(loc_vers))## remove duplicates
        #loc_vers.reverse()
        for v in loc_vers:
...
def pre_filter_cand_list_new(candidate_list, versions):
    # A: Find optimum for each candidate
    # #1: Get first entry
    # #2: If an entry is split further down, abort
    # #3
    ...
    for entry in candidate_list:
        brea = False
        for e in entry[0]:
            # update to more specific version if entry did not split
            if e in entries:
                names[entries.index(e)] = entry[1]
                continue
            else:
                # if a pli got split, abort
                for ent in entries:
                    for index in e:
                        if index in ent:
                            brea = True
                ...
                # if pli is not in entries and did not get split, add it to entries
                entries.append(e)
                names.append(entry[1])

    new_candidate_list = []
    # Do a second pass over the list and add every entry in entries and names to the new list:
    k = 0
    for cand in candidate_list:
        entr = []
        for i in range(0, len(entries)):
            if names[i] == cand[1]:
                entr.append(entries[i])
        new_cand = [entr, cand[1], cand[2]]
        if len(entr) > 0:
