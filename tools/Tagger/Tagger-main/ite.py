...
def find_ite(nodes, depth):
    ...
    for i in range(0, len(clmns)):
        ...
        if (config["ignore_constant_attributes"] and not config["disable_heuristics"] and len(
                pli) == 1):  # or all(p == [-1] for p in pli):
            continue
...
def update_schema(schema, tree):
    ...
    current_schema_global["description"] = f"Discovered {ite_count} if-then-else constraints"
...
def run_experiments(config_path):
    ...
    for exp_key in exp_config:
        ...
        for i in range(len(configs)):
            ...
            extracted_schema = experiments.get_schema()
            sys.stdout = save_stdout

            res_str, size_base, size_total = update_schema(extracted_schema, tree)
