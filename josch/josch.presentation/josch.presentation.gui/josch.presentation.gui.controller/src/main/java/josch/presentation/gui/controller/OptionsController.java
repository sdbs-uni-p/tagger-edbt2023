...
public class OptionsController extends AbstractController {
  ...
  @FXML private TextField folderTools;

 @FXML private TextField folderSPO;

 @FXML private TextField folderFRO;

 @FXML
 private TextField folderJava8;

 @FXML private GridPane folderList;
  ...
  private void connect() {
    ...
    if (!(isIjsInstalled && isJssInstalled)) {
      ...
      setSettings(settingsDto);
        ...
        // Starts the frozza server here once - shutdown is in shutdownhook
        FrozzaExtractionService frozzaExtractionService
                = new FrozzaExtractionService(getSettings(), new TaggerExtractionDto());
  ...
  /** */
  @FXML
  private void chooseToolsFolder() {
    chooseFolder();
  }

  @FXML
  private void chooseFrozzaFolder() {
      // Open a File chooser Dialog to select the directory.
      DirectoryChooser directoryChooser = new DirectoryChooser();
      directoryChooser.setTitle("Select the Location of the approach of frozza et al.");
  ...
  @FXML
  private void chooseJava8Folder() {
      // Open a File chooser Dialog to select the directory.
      DirectoryChooser directoryChooser = new DirectoryChooser();
      directoryChooser.setTitle("Select the Location of java 8 for the Approach for Spoth et al.");
      File directory = directoryChooser.showDialog(new Stage());

      // Set the path.
      TextField textField = new TextField();
      textField = folderJava8;
      if (directory != null && directory.exists()) {
          textField.setText(directory.getAbsolutePath());
      }
  }
  ...
  private void chooseFolder() {
    // Open a File chooser Dialog to select the directory.
    DirectoryChooser directoryChooser = new DirectoryChooser();
    directoryChooser.setTitle("Select the Location of the Tools directory");
    File directory = directoryChooser.showDialog(new Stage());

    // Set the path.
    TextField textField = new TextField();
    textField = folderTools;
    if (directory != null && directory.exists()) {
      textField.setText(directory.getAbsolutePath());
    }
  }
  ...
  private void setToolDirectories(SettingsDto settings) {
    ...
    settings.setToolsPath(folderTools.getText());
    settings.setJava8Path(folderJava8.getText());
    settings.setFrozzaPath(folderFRO.getText());
  }
  ...
  public void initialize(URL location, ResourceBundle resources) {
    ...
    folderTools.setText(preferences.get("toolsPath", ""));
    folderJava8.setText(preferences.get("java8Path", ""));
    folderFRO.setText(preferences.get("frozzaPath", ""));
  ...
  private void saveSettings() {
    if (cbSave.isSelected()) {
      ...
      preferences.put("toolsPath", folderTools.getText());
      preferences.put("java8Path", folderJava8.getText());
      preferences.put("frozzaPath", folderFRO.getText());
    }
  }
}
