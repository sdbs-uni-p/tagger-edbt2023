...
public class CollectionViewController extends AbstractDatabaseController {
  ...
  @FXML
  private Tab tabTagger;

  // the checkboxes

  @FXML
  private CheckBox btnIgnoreConstant;

  @FXML
  private CheckBox btnIgnoreUnique;

  @FXML
  private CheckBox btnUseUnion;
  ...
  @FXML
  private IntegerTextField taggerThreshold;

  @FXML
  private CodeArea tagRightCodeArea;

  @FXML
  private ComboBox<String> valueAbsoluteRelative;

  @FXML
  private TextField size;

  @FXML
  private Label extractUnit;

  @FXML
  private Label taggerPercentLabel;

  @FXML
  private CodeArea taggerLeftCodeArea;
  /**
   *
   * {@inheritDoc}
   */
  ...
  public void initialize(URL url, ResourceBundle rb) {
    ...
    initializeCodeArea(tagRightCodeArea);
    initializeCodeArea(taggerLeftCodeArea);
    ...
    valueAbsoluteRelative.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
      if (newValue.equals("absolute")) {
        extractUnit.setText("documents");
      } else if (newValue.equals("relative")) {
        extractUnit.setText("%");
      }
    });
  ...
  /**
   * Validates the document in the input field against the schema in the tagger field.
   */
  @FXML
  private void validateTaggerDocument() {
    // Get the document, schema and service.
    String schema = tagRightCodeArea.getText();
    String document = taggerLeftCodeArea.getText();
    IValidationService service = getValidationService();
  ...
  @FXML
  private void validateTaggerAll() {
      // Get the validator and the service.
      String schema = tagRightCodeArea.getText();
      IValidationService service = getValidationService();

      // Validate and print the message.
     ValidationDto validationDto = service.validate(schema);
     if (validationDto.getAmountInvalidDocuments() == 0) {
       notifyUser("The schema validates against all documents");
     } else {
       notifyUser("The schema does not validate against " + validationDto.getAmountInvalidDocuments() + " documents." +
               "These are " + validationDto.getInvalidDocuments());
     }
  }
...
//      System.out.println("-----");
  ...
  private String taggerSchema;


  private Callable<String> processExtract() {
    return (() -> {
        String jsonSchema;
        boolean ignoreConstant = btnIgnoreConstant.isSelected();
        boolean ignoreUnique = btnIgnoreUnique.isSelected();
        boolean useUnion = btnUseUnion.isSelected();

        String algorithm = algorithmsComboBox.getSelectionModel().getSelectedItem();
        int taggerThresholdPercent = Integer.parseInt(taggerThreshold.getText());
        float taggerThresholdFloat = taggerThresholdPercent / 100f;
        // look at process introspect for that
        ExtractionDto extractionDto = getSettings().getExtraction();
        extractionDto.setSize(Integer.parseInt(size.getText()));
        extractionDto.setMethod(valueAbsoluteRelative.getValue());

        TaggerExtractionDto taggerExtractionDto = new TaggerExtractionDto();
        taggerExtractionDto.setSize(Integer.parseInt(size.getText()));
        taggerExtractionDto.setMethod(valueAbsoluteRelative.getValue());
        IExtractionService extractionService =
                AbstractServiceFactory.getTaggerExtractionService(getSettings(), taggerExtractionDto, algorithm);

        taggerExtractionDto.setIgnoreUnique(ignoreUnique);
        taggerExtractionDto.setIgnoreConstantAttributes(ignoreConstant);
        taggerExtractionDto.setUseUnion(useUnion);
        taggerExtractionDto.setSampleSize(taggerThresholdFloat);
        jsonSchema = extractionService.getJsonSchema();
        this.taggerSchema = pretty(jsonSchema);
        return null;
    });
  }
  ...
  public String extract() {
    try {
      execute(processExtract());
      this.rightSchema = taggerSchema;
      this.tagRightCodeArea.replaceText(taggerSchema);
      notifyUser("Your schema was updated");
      return taggerSchema;
    } catch (Exception e) {
      notifyUser("Something went wrong in the extraction process. Please check your input and try again.");
      return null;
    }
  }
  ...
  @FXML
  private void getSampleTagger() {
    // load a random document.
    IDatabaseService service = getDbService();
    String document = service.getRandomDocument();

    // show the random document as a pretty JSON String.
    taggerLeftCodeArea.replaceText(pretty(document));
  }
