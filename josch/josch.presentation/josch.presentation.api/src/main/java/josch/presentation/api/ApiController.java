...
//      run(iterations, sampleSize, EExtractionTools.JSI, false);
     // run(iterations, sampleSize, EExtractionTools.HACK, false);
      run(iterations, sampleSize, EExtractionTools.KLETTKE, false);
}
  }
   ...
   */
  private static void run(int iterations, int sampleSize, EExtractionTools extraction, boolean skip) throws IOException {
  ...
  private static double setup(SettingsDto settings, int sampleSize) {
    ...
    ConnectionInfoDto localhost = new ConnectionInfoDto("mongodb://localhost:27017/test");
    ...
    ijs.setCollection(databaseService.getCollection("mapGermany"));
    jss.setCollection(databaseService.getCollection("mapGermany"));
    ...
    extractionParams.setContainer("test");
    extractionParams.setEntity("mapGermany");
