...
public class ValidationService implements IValidationService {
    ...
    if (schema.contains("$jsonSchema")) {
      ...
      // If this is a JSON Schema according to drafts
        ...
        && (schema.contains("json-schema.org/draft-04/schema")
            ...
            || schema.contains("json-schema.org/draft-07/schema")) || true) {
      try {
        SchemaStore schemaStore = new SchemaStore();
        Schema schemaO = schemaStore.loadSchemaJson(schema);
        Validator validator = new Validator();
        validator.validateJson(schemaO, document);
        message = "The schema validates this document.";
      } catch (SchemaException | JSONException e) {
        //e.printStackTrace();
        if (e.getMessage().contains("com.fasterxml.jackson.core.JsonParseException")) {
          message = ESystemConstants.ERROR.getValue() + "Syntax error";
        } else {
          System.out.println(ANSI_RED + e.getMessage()+ ANSI_RESET);
          message = ESystemConstants.ERROR.getValue() + "The schema does not validate this document"; //e.getMessage();
        }
      }
    } else {
  ...
  public ValidationDto validate(String schema) {
    ...
    if (settings.getDbms().equals(EDatabaseSystems.MONGO) && isMongoSchema(schema)) {
    ...
    } else if (isJSONSchema(schema) || true) {
      ...
      try {
        SchemaStore schemaStore = new SchemaStore();
        Schema jsonSchema = schemaStore.loadSchemaJson(schema);
        Validator validator = new Validator();
        ...
        do {
          ...
          if (currentDocument != null) {
            ...
            try {
              validator.validateJson(jsonSchema, currentDocument);
